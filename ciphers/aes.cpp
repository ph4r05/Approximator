...
void AddRoundKey(uchar state[][4], const uint w[])
{
   uchar subkey[4];
...
{
   const int numRounds = AES_ROUNDS(keysize);
   int rnd;
   uchar state[4][4];
   if (rounds<0) {
       rounds = numRounds;
   ...
   for(rnd=0; (rnd+1)<rounds; rnd++){
        SubBytes(state); ShiftRows(state); MixColumns(state); AddRoundKey(state,&key[(rnd+1) * 4]);
   }
   
   // Final round
   if (rnd < rounds){
    SubBytes(state); ShiftRows(state); AddRoundKey(state,&key[(rnd+1) * 4]);
   }
...
void aes_decrypt(const uchar in[], uchar out[], const uint key[], int keysize, int rounds)
{
   ...
   // Perform the necessary number of rounds. The round key is added first.
   // The last round does not perform the MixColumns step.
   AddRoundKey(state,&key[4*numRounds]);
   for(rnd=0; (rnd+1)<rounds; rnd++){
       InvShiftRows(state);InvSubBytes(state);AddRoundKey(state,&key[4*numRounds-(rnd+1)*4]);InvMixColumns(state);
   }
