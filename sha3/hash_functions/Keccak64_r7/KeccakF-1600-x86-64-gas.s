...
.macro	mKeccakCopyState	iState, oState
                ...
                xorq            %rcx, %rcx
loop1\@:             
                movq            (\iState, %rcx, 8), %rax
                movq            %rax, 0(\oState, %rcx, 0x8)
                addq            $1, %rcx
                cmp             $25, %rcx
                jnz             loop1\@
loopEnd\@:        
...
.macro	mKeccakPermutationInit	

                subq		$8*25, %rsp                     # %rsp = rpStack
...
.macro	mKeccakPermutation2	
                # Keccak permutation initialization
                mKeccakPermutationInit
                
                # Keccak rounds
		mKeccakRound	rpState, rpStack, 0x0000000000000001, 0
		mKeccakRound	rpStack, rpState, 0x0000000000008082, 1

		addq		$8*25, %rsp

		.endm

.macro	mKeccakPermutation3	
                # Keccak permutation initialization
                mKeccakPermutationInit
...
.macro	mKeccakPermutation8	
		...
		mKeccakRound	rpStack, rpState, 0x8000000000008009, 1
